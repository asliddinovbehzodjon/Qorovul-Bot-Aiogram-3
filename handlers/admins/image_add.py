...
from states.mystate import ImageState
from aiogram.fsm.context import FSMContext
from aiogram.utils.keyboard import InlineKeyboardBuilder
from helper import check_url
@dp.message(F.text=='📸 Rasm',IsChatAdmin(),IsPrivate())
async def start_create_text_post(message:types.Message,state:FSMContext):
        await message.answer("<b>Post rasmi va tekstini yuboring.</b>",reply_markup=back_button())
...
@dp.message(F.text=='◀️ Orqaga',IsChatAdmin(),ImageState.image,IsPrivate())
...
@dp.message(ImageState.image,IsChatAdmin(),IsPrivate())
async def get_text(message:types.Message,state:FSMContext):
    ...
    if content_type=='photo':
            ...
            await state.set_state(ImageState.url)
    else:
            await message.answer("<b>Post rasmi va tekstini yuboring.</b>", reply_markup=back_button())
            await message.answer("Post sozlamalari:", reply_markup=text_format())
            await state.set_state(ImageState.image)
@dp.message(F.text=="🆗 Kerakmas",ImageState.url,IsChatAdmin(),IsPrivate())
async def pass_to_url(message:types.Message,state:FSMContext):
    data = await state.get_data()
    await message.answer_photo(photo=data['file_id'],caption=data.get('caption'))
    await message.answer("Agar tayyor bo'lsa '📤 Yuborish' tugmasini bosing!", reply_markup=send())
    await state.set_state(ImageState.check)
# Cancel
@dp.message(F.text=="⏺ Bekor qilish",ImageState.url,IsChatAdmin(),IsPrivate())
async def pass_to_url(message:types.Message,state:FSMContext):
        await message.answer("Qaysi turdagi xabar yuborasiz!\n"
                             "Tanlang👇", reply_markup=add_type())
        await state.clear()
# Cancel 2
@dp.message(F.text=="⏺ Bekor qilish",ImageState.check,IsChatAdmin(),IsPrivate())
async def pass_to_url(message:types.Message,state:FSMContext):
        await message.answer("Qaysi turdagi xabar yuborasiz!\n"
                             "Tanlang👇", reply_markup=add_type())
        await state.clear()
@dp.message(ImageState.url,IsChatAdmin(),IsPrivate())
async def get_link(message:types.Message,state:FSMContext):
    if message.content_type=='text':
        ...
        await message.answer_photo(photo=data['file_id'],caption=data.get('caption',None),reply_markup=btn.as_markup(row_width=1))
        await message.answer("Agar tayyor bo'lsa '📤 Yuborish' tugmasini bosing!",reply_markup=send())
        await state.set_state(ImageState.check)
    else:
            text = "Havolani quyidagi formatda yuborish:\n" \
            ...
            await message.answer(text, reply_markup=need_or_not())
            await state.set_state(ImageState.url)
@dp.message(F.text=="📤 Yuborish",ImageState.check,IsChatAdmin(),IsPrivate())
async def check_post(message:types.Message,state:FSMContext):
    ...
    if data.get('buttons',None):
        ...
        for i in USERS:
            try:
                await bot.send_photo(photo=photo,chat_id=i['telegram_id'], caption=caption,reply_markup=btn.as_markup(row_width=1))
                counter += 1
            except:
                pass
        ...
        for i in USERS:
            try:
                await bot.send_photo(photo=photo,chat_id=i['telegram_id'],caption=caption)
                counter+=1
            except:
                pass
        await message.answer(f"{counter} kishiga xabar yuborildi!",reply_markup=admin_button())
